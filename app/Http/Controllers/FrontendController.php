...
class FrontendController extends Controller
{
    public function index(){
        return view('frontend_pages.home', [
            ...
            'latest_items'  => Item::latest()->limit(6)->get(),
            'all_items'     => Item::all(),
            'items'         => Item::all(),
            'reports'       => ItemReport::all(),
        ]);
    }

    public function search_item(Request $request){
        $all_items = Item::where('name','LIKE', '%'.$request->search_item.'%')->latest()->get();
    ...
    public function search_by_day($day){
        ...
        return view('frontend_pages.home', [
            ...
            'newsletter'    => Newsletter::first(),
            'latest_items'  => Item::latest()->limit(6)->get(),
            'all_items'     => Item::where('created_at', '>=', $date)->get(),
            'items'         => Item::all()
        ]);
    ...
    public function filterByCategoryCollection(Request $request){
        $all_items = Item::where('category_id',$request->cat_id)->get();
        $view = view('frontend_pages.resources.filterByExplore', compact('all_items'));
    ...
    public function RecentlyAdded(Request $request){
        $items = Item::latest()->limit(5)->get();
        $view = view('frontend_pages.resources.filterByCategory', ['reports' => ItemReport::all(), 'items' => $items]);
    ...
    public function allItemsUser(Request $request){
        $user_items = Item::where('creator_id', $request->user_id)->get();
    ...
    public function RecentlyAddedUser(Request $request){
        $user_items = Item::where('creator_id', $request->user_id)->latest()->get();
    ...
    public function LowToHighUser(Request $request){
        $user_items = Item::where('creator_id', $request->user_id)->orderBy('price','ASC')->get();
    ...
    public function HighToLowUser(Request $request){
        $user_items = Item::where('creator_id', $request->user_id)->orderBy('price','DESC')->get();
    ...
    public function RecentlyAddedCollection(Request $request){
        $items = Item::where('category_id', $request->category_id)->latest()->get();
        $reports = ItemReport::all();

        $data = view('frontend_pages.resources.onSaleCategory', compact('items','reports'))->render();

        return response()->json(['data'=> $data]);
    }

    public function LowToHighCollection(Request $request){
        $items = Item::where('category_id', $request->category_id)->orderBy('price','ASC')->get();
        $reports = ItemReport::all();
        $data = view('frontend_pages.resources.onSaleCategory', compact('items','reports'))->render();
    ...
    public function HighToLowCollection(Request $request){
        $items = Item::where('category_id', $request->category_id)->orderBy('price','DESC')->get();
        $reports = ItemReport::all();
        $data = view('frontend_pages.resources.onSaleCategory', compact('items','reports'))->render();
    ...
    public function item_details($slug){
        ...
        $related_items = Item::where('category_id' , $single_item->category_id)->where('id','!=',$single_item->id)->get();
        ...
        $itemproperties = ItemProperty::where('item_id', $single_item->id)->get();

        return view('frontend_pages.pages.item_details',compact('single_item','related_items','max_bid','latest_bid', 'reports','itemproperties'));
    }
    ...
    public function user($id){
        return view('frontend_pages.pages.user',[
            'user' => User::find($id),
            'user_items' => Item::where('creator_id', $id)->get(),
            'reports' => ItemReport::all(),
        ]);
    }
    public function user_search_all_item($user_id){
        $user_items = Item::where('creator_id', $user_id)->get();
        if($user_items){
            return view('frontend_pages.pages.user',[
    ...
    public function filterCategoryByRange(Request $request){
        $user_items = Item::where('creator_id', $request->user_id)->whereBetween('price',[$request->min_amount, $request->max_amount])->get();
        $data = view('frontend_pages.resources.filterByUser', compact('user_items'))->render();
        return response()->json(['data' => $data]);
    }
    ...
    public function filterByCategoriesInUser(Request $request){
        // dd($request->user_id);
        $user_items = Item::where('category_id',$request->cat_id)->where('creator_id', $request->user_id)->get();
    ...
    public function filterByPriceRangeInCollection(Request $request){
        ...
        $items = Item::where('category_id', $request->category_id)->whereBetween('price',[$request->min_amount, $request->max_amount])->get();
        $reports = ItemReport::all();
        $data = view('frontend_pages.resources.onSaleCategory', compact('items','reports'))->render();
    ...
    public function search_by_price(Request $request){
        $items = Item::whereBetween('price',[$request->min_amount, $request->max_amount])->where('category_id', $request->cat_id)->get();
        return view('frontend_pages.pages.collection',[
            'items' => $items,
            'nftcategory' => NFTCategory::find($request->cat_id),
        ]);
